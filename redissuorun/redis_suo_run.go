// Package redissuorun: High-grade distributed lock package with automatic reattempt and lifecycle management
// Provides convenient lock acquisition using built-in reattempt approach, timeout handling, and guaranteed cleanup
// Features automatic lock release, panic handling, and context-aware execution management
// Designed serving robust production environments needing correct distributed coordination
//
...
// Handles lock acquisition reattempts, guaranteed lock release, and panic handling
// Provides complete lifecycle management in distributed lock operations
// Returns errors when context cancellation happens and business logic fails
//
...
// Used through reattempt approach achieving persistent lock acquisition
//
...
// retryingAcquire keeps attempting lock acquisition before success and context cancellation
...
// Creates safe context with minimum timeout ensuring release completion
// Returns true on completing release, false if owned through a different session
// Used through reattempt approach achieving guaranteed lock cleanup
//
...
// safeCtx creates a safe context in operations even when parent context is cancelled
...
// Needed achieving guaranteed cleanup operations regardless of parent context state
//
...
// execRun executes business logic within timeout constraints with panic handling
// Creates timeout context based on remaining lock TTL achieving safe execution
// Delegates to safeRun during comprehensive problems and panic handling
